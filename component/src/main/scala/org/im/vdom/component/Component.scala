...
object Component {
  ...
  protected[this] def internalGetID(node: dom.Node): Option[NodeIDType] = {
    node match {
      case e: dom.Element => Some(e.getAttribute(ID_ATTRIBUTE_NAME))
      case _ => None
    }
  }
   ...
   */
  def getID(node: dom.Node): Option[NodeIDType] = {
    // extract from node
  ...
  val instancesByRootID: collection.mutable.Map[Int, Component[_]] =
    collection.mutable.Map()
  val containerByRootID: collection.mutable.Map[Int, dom.Node] =
    collection.mutable.Map()

  /**
   * Register the container. Create root ID if needed.
   */
  def registerContainer(container: dom.Node): Unit = {
  }

  /**
   * Render a component into a container.
   */
  def render(component: Component[_], container: dom.Node): Unit = {
  }
